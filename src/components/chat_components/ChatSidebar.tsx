...
function CustomModal({
  ...
  const handleConfirm = async () => {
    ...
    try {
      setProcessing(true);
      // support sync or async onConfirm
  ...
  return (
    ...
    >
      <div
        className="bg-white lg shadow-xl max-w-md w-full mx-4 relative"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Top-right cross (keeps same design/position) */}
        <button
          onMouseDown={(e) => {
            // immediate close to avoid tiny focus/blur delays
            e.preventDefault();
            // ensure processing is not left true (defensive)
            setProcessing(false);
            onClose();
          }}
          aria-label="Close"
          className="absolute right-5 top-4 text-gray-400 hover:text-gray-800 p-1 rounded-full"
        >
          <X className="w-5 h-5" />
        </button>
          ...
          <div className="flex gap-3 justify-end">
            {cancelText && (
              <button
                onMouseDown={(e) => {
                  ...
                  e.preventDefault();
                  setProcessing(false); // ensure not stuck
                  onClose();
                }}
            ...
            {confirmText && onConfirm && (
              <button
                ...
                disabled={processing}
                className={cn(
                  "px-4 py-2 rounded-lg font-semibold text-white flex items-center justify-center",
                  type === "danger"
              ...
              >
                {processing && (
                  // small spinner (white) shown when processing
  ...
  const renderChatRow = (chat: ChatItem, isArchived = false) => {
    ...
    return (
      <div
        ...
        onClick={(e) => {
          // If the click originated from inside the dropdown menu content, ignore it.
          const target = (e.target as HTMLElement) || null;
          if (
            target &&
            target.closest &&
            target.closest(".rc-dropdown-content")
          ) {
            return;
          }

          // Otherwise treat as a normal chat row click
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  size="icon"
                  className="opacity-0 group-hover:opacity-100 hover:bg-teal-100 transition-opacity h-6 w-6"
                  onClick={(e) => e.stopPropagation()}
              ...
              <DropdownMenuContent
                side="right"
                align="start"
                className="rc-dropdown-content p-0 bg-white rounded-md shadow-md"
              >
                <DropdownMenuItem asChild>
                  <button
                    type="button"
                    className="block w-full text-left px-3 py-2 text-gray-700 hover:bg-teal-100 focus:outline-none first:rounded-t-md last:rounded-b-md"
                    onClick={() => {
                ...
                <DropdownMenuItem asChild>
                  <button
                    type="button"
                    className="block w-full text-left px-3 py-2 text-red-600 focus:bg-red-100 focus:text-red-600 focus:outline-none"
                    onClick={() => {
