...
export default function ChatWindow({ chatId, onNewChat }: Props) {
  ...
  const { draftMessage, setDraftMessage } = useChatContext();
  ...
  const welcomeMessage: Message = {
    role: "assistant",
    text: "Welcome to EyeCare's support — Our assistant can help interpret results, explain next steps, and guide you to relevant resources.",
  ...
  // Click detection - focus only when clicking input area, not messages
  useEffect(() => {
    const onMouseDown = (e: MouseEvent) => {
      ...
      // Check if clicked on the input or its parent form
      const clickedInput =
        input.contains(e.target as Node) ||
        input.parentElement?.contains(e.target as Node);

      if (clickedInput) {
        // Clicked on input area -> enable autofocus and focus
        setAllowAutoFocus(true);
        setTimeout(() => {
          inputRef.current?.focus();
        }, 0);
      } else if (!root.contains(e.target as Node)) {
        // Clicked completely outside the chat window -> disable autofocus
        setAllowAutoFocus(false);
      }
      // If clicked inside messages area, do nothing (allow text selection)
    };
  ...
  async function send(e: React.FormEvent) {
    e.preventDefault();
    if (!draftMessage.trim() || loading) return;

    const inputText = draftMessage;
    setDraftMessage(""); // Clear after sending
    ...
    try {
      ...
      const errorMessage: Message = {
        ...
        text:
          err?.status === 401
            ? "⚠️ Unauthorized. Please log in again."
            : "⚠️ Please Sign Up or Login to continue.",
        timestamp: new Date().toISOString(),
      };
