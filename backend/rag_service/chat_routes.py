...
async def get_embedding_cached(text: str):
    """Get embedding with caching to avoid regenerating for similar queries"""
    cache_key = hash(text[:200])  # Use first 200 chars as cache key
    current_time = time.time()
    
    # Check cache
    if cache_key in _query_embedding_cache:
        cached_embedding, timestamp = _query_embedding_cache[cache_key]
    ...
    try:
        ...
        _query_embedding_cache[cache_key] = (embedding, current_time)
        return embedding
...
async def chat_endpoint(req: ChatRequest, user=Depends(get_current_user)):
    try:
        ...
        try:
            # Use cached embedding function for faster queries
            query_embedding = await get_embedding_cached(req.message)
...
"""
        ...
        try:
            ...
            resp = genai_client.generate_content(final_prompt)
